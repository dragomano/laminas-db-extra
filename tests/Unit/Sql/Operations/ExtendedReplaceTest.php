...
describe('ExtendedReplace', function () {
    beforeEach(function () {
        ...
        $this->replace = new ExtendedReplace();
    });

    describe('inheritance from ExtendedInsert', function () {
        it('inherits ExtendedInsert functionality', function () {
            expect($this->replace)->toBeInstanceOf(ExtendedReplace::class);
        ...
        it('supports table prefixes', function () {
            $replace = new ExtendedReplace('test', 'prefix_');
    ...
    describe('executeReplace', function () {
        it('executes REPLACE INTO for MySQL', function () {
            ...
            $this->adapter->shouldReceive('query')
                ...
                ->with(
                    /** @lang text */
                    'REPLACE INTO `test_table` (`id`,`name`,`email`) VALUES (?,?,?)',
        ...
        it('returns ResultSetWrapper when returning is set and result is ResultSet', function () {
            $this->replace->into('test_table');
            $this->replace->columns(['id', 'name', 'email']);
            $this->replace->values([1, 'John Doe', 'john@example.com']);
            $this->replace->setReturning(['id', 'name', 'email']);
            ...
            $this->adapter->shouldReceive('query')
                ->once()
                ->andReturnUsing(function ($sql) use ($resultSet) {
    ...
    describe('executeBatchReplace', function () {
        ...
        it('returns ResultSetWrapper for MySQL batch when returning is set', function () {
            $batchData = [
                ['id' => 1, 'name' => 'John Doe', 'email' => 'john@example.com'],
                ['id' => 2, 'name' => 'Jane Doe', 'email' => 'jane@example.com'],
            ];
    ...
    describe('ExtendedInsert methods integration', function () {
    ...
    describe('table prefix functionality', function () {
        ...
        it('applies prefix to table name in executeBatchReplace', function () {
            $replace = new ExtendedReplace('users', 'prefix_');
